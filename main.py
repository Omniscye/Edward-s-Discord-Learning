...
class DoorQuizGame:
    def __init__(self):
        ...
        try:
            ...
            pygame.mixer.music.set_volume(0.1)  # Set background music volume (10%)
        except pygame.error:
    ...
    async def game_over_animation(self):
        """Play game over animation asynchronously."""
        ...
        for i in range(100):
            ...
            pygame.display.flip()
            await asyncio.sleep(0.05)  # Allow browser to process frames
        await self.display_game_over_text()

    async def display_game_over_text(self):
        """Display 'Game Over' text asynchronously."""
        self.screen.fill(self.BLACK)
                    ...
                    for i, answer_rect in enumerate(answer_rects):
                        if answer_rect.collidepoint(mouse_pos):
                            if i == self.questions[self.current_question]["correct"]:
                                ...
                                if self.wrong_answer_sound and not pygame.mixer.get_busy():
                                    self.wrong_answer_sound.play()
                                self.lives -= 1
                                if self.lives <= 0:
                                    await self.game_over_animation()
                                    running = False
